...
from bot.helper.ext_utils.bot_utils import get_bot_pm, is_paid, is_sudo, getdailytasks, get_user_task, getUserTDs, is_url, \
        ...
        is_udrive_link, is_sharer_link, is_sharedrive_link, is_filepress_link, is_unified_link, new_thread
...
def _mirror_leech(bot, message, isZip=False, extract=False, isQbit=False, isLeech=False):
    ...
    if not is_url(link) and not is_magnet(link):
        msg = f"It's Invalid Link , Please Try Later\n\n"
        reply_message = sendMessage(msg, bot, message)
        Thread(target=auto_delete_message, args=(bot, message, reply_message)).start()
        return reply_message
...
def start_ml(extra, s_listener):
    ...
    if is_gdrive_link(link):
        if not isZip and not extract and not isLeech:
            ...
            Thread(target=add_gd_download, args=(link, f'{DOWNLOAD_DIR}{listener.uid}', listener, name, is_gdtot, is_udrive, is_sharer, is_sharedrive, is_filepress, is_unified)).start()
